...
const doGenerateSiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`, dataFormSend);
...
const doDeleteProsesSiswa = async (id, proses_id) => {
    if (confirm("Apakah anda yakin menghapus data ujian ini?")) {
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.delete(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`);
...
const do_reset_waktu_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/waktu`, dataFormSend);
            console.log(response?.data);
...
const do_reset_salah_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/salah`, dataFormSend);
            console.log(response?.data);
...
const do_reset_all_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/all`, dataFormSend);
            console.log(response?.data);
...
const doUpdateTanggalPerkelas_v4 = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/update/tgl`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doUpdateTanggalPersiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/update/tgl`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
        ...
        <div>
            ...
            <div class="w-full bg-base-100 shadow-sm rounded-lg py-3 px-4">
                <div class="flex items-center justify-between">
                    <!-- Judul -->
                    <div>
                        <h1 class="text-lg font-bold">UJIAN HSPQ : KELAS {{ kelas_nama }}</h1>
                        <h5 class="text-sm text-gray-500">Hanya untuk ujian HSPQ Saja</h5>
                    </div>
            ...
            <div class="bg-base-100 shadow-sm rounded-lg p-4 space-y-4">
                ...
                <div class="flex items-center justify-between">
                    <p class="text-sm">
                        ...
                        <span class="ml-2">Ujian Psikologi HSPQ || label : hspq_only || id :
