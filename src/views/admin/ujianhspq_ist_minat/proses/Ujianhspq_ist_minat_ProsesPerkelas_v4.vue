...
const getData = async () => {
    try {
        ...
        console.log('====================================');
        isLoading.value = true;
        const response = await ApiUjianKhusus.get(`/ujian/hspq_ist_minat/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`);
...
const doGenerateSiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`, dataFormSend);
...
const doDeleteProsesSiswa = async (id, proses_id) => {
    if (confirm("Apakah anda yakin menghapus data ujian ini?")) {
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.delete(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`);
...
const do_reset_waktu_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/waktu`, dataFormSend);
            console.log(response?.data);
...
const do_reset_salah_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/salah`, dataFormSend);
            console.log(response?.data);
...
const do_reset_all_persiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/reset/umum/all`, dataFormSend);
            console.log(response?.data);
...
const doGenerateSiswaPerkelas = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doUpdateTanggalPerkelas_v4 = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/update/tgl`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doUpdateTanggalPersiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/update/tgl`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doDeleteProsesSiswaPerkelas = async (id) => {
    if (confirm("Apakah anda yakin menghapus data ujian ini?")) {
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.delete(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`);
            Toast.babeng("Berhasil", 'Data berhasil di hapus!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doGenerateHasilPerkelas_v4 = async (replace = true) => {
    if (confirm("Apakah anda yakin generate Hasil Kelas Ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`/ujian/hspq_ist_minat/proses/generate/sekolah/${getSekolahAktif.value.sekolah_id}/kelas/${getSekolahAktif.value.kelas_id}/generate/hasil`, { replace: replace });
            Toast.babeng("Berhasil", 'Generate Hasil Ujian telah berhasil!');
            getData();
            return true;
        } catch (error) {
...
const doGenerateHasilPersiswa_v4 = async (siswa_id) => {
    if (confirm("Apakah anda yakin generate Hasil Kelas Ini?")) {
        ...
        try {
            ...
            const response = await ApiUjianKhusus.post(`ujian/hspq_ist_minat/proses/generate/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${siswa_id}/generate/hasil`, dataForm);
            Toast.babeng("Berhasil", 'Generate Hasil Ujian telah berhasil!');
            getData();
            return true;
        } catch (error) {
        ...
        <div>
            ...
            <div class="w-full bg-base-100 shadow-sm rounded-lg py-3 px-4">
                <div class="flex items-center justify-between">
                    <!-- Judul -->
                    <div>
                        <h1 class="text-lg font-bold">UJIAN HSPQ + IST + MINAT: KELAS {{ kelas_nama }}</h1>
                        <h5 class="text-sm text-gray-500">Hanya untuk ujian HSPQ + IST + MINAT</h5>
                    </div>
            ...
            <div class="bg-base-100 shadow-sm rounded-lg p-4 space-y-4">
                ...
                <div class="flex items-center justify-between">
                    <p class="text-sm">
                        ...
                        <span class="ml-2">Ujian Psikologi HSPQ || label : ujian_hspq_ist_minat || id :
                            68df4eca64689acb8dcc9d7b</span>
                    </p>
                ...
                <div class="rounded-lg bg-base-200 p-4 text-sm leading-relaxed">
                    ...
                    <ul class="list-disc ml-5 space-y-1">
                        <li>Sebelum mulai, pastikan paket sudah ada di menu Paket soal ujian khusus / <b>ujian IQ</b>.
                        </li>
                ...
                <div class="flex items-start gap-3">
                    ...
                    <div class="flex flex-wrap items-center gap-2 grow">
                        <button class="btn btn-sm btn-primary" @click="doGenerateSiswaPerkelas()">
                            Generate UJIAN PERKELAS + CACHING 1 + CACHING 2
                        </button>
         ...
         hover:bg-emerald-600  tooltip" data-tip="Generate UJIAN + CACHING 1 + 2 "
                                        ...
                                        <span v-else-if="props.column.field == 'tgl_batas_mulai'">
                                            <!-- {{ props.row.progres?.created_at }} -->
                                            {{ props.row.ujian_hspq_ist_minat?.tgl_batas_mulai ?
                                                moment(props.row.ujian_hspq_ist_minat?.tgl_batas_mulai).format(formatTanggal)
                                                : "-" }}
                                        </span>
