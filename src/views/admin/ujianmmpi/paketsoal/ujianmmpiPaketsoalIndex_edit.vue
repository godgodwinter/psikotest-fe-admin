...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`mmpi/paketsoal/${paketsoal_id.value}`);
...
const onSubmit = async (values) => {
    ...
    try {
        const response = await ApiUjianKhusus.put(`mmpi/paketsoal/${paketsoal_id.value}`, dataFormSend);
        // console.log(response);
        Toast.success("Info", "Data berhasil diupdate!");
        router.push({ name: "admin-ujianmmpi-paketsoal" });
        return true;
    } catch (error) {
        console.error(error);
    }
