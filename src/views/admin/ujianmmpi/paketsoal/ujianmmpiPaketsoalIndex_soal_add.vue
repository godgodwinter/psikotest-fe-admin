...
const getData = async () => {
    try {
        const response = await ApiUjianKhusus.get(`mmpi/banksoal`);
...
const doAdd = async () => {
    ...
    const response = await ApiUjianKhusus.post(`mmpi/paketsoal/${paketsoal_id.value}/add_soal`, dataForm.value);
