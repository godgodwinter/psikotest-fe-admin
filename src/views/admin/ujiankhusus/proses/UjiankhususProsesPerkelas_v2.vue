...
const getData = async () => {
    try {
        isLoading.value = true;
        const response = await ApiUjianKhusus.get(`/ujiankhusus/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/v2`);
...
const doGenerateSiswaPerkelas = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`ujiankhusus/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
        ...
        <div>
            ...
            <div class="space-x-2 shadow-sm">
                ...
                <button class="btn btn-sm btn-info p-2" @click="doGenerateSiswaPerkelas()">
                    Generate UJIAN KHUSUS Per Kelas
                </button>
