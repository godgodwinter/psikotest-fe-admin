...
const onSubmit = async (values) => {
    ...
    if (doSubmit > 0) {
        let inputForm = {
            ...
            skor_jawaban_benar: values.skor_jawaban_benar,
            urutan: values.urutan,
            status: "Aktif"
        }
        // console.log(inputForm);
        const response = await ApiUjianKhusus.post(
            `cfit/banksoal/aspek_detail/${aspek_detail_id.value}/soal`,
            inputForm
        );
...
<template>
    <div>
        <article class="prose lg:prose-sm">
            <h1>MULTIJAWABAN SOAL : TAMBAH</h1>
