...
const getData = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/banksoal/aspek_detail/${aspek_detail_id.value}/soal`);
...
const doDeleteData = async (id, index) => {
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            const response = await ApiUjianKhusus.delete(`cfit/banksoal/aspek_detail/${aspek_detail_id.value}/soal/${id}`);
            if (response.status) {
