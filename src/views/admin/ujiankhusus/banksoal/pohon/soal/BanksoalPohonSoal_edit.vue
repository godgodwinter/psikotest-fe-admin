...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/v2/banksoal/pohon/${aspek_detail_id.value}/soal/${soal_id.value}`);
...
const onSubmit = async (values) => {
    ...
    if (doSubmit > 0) {
        ...
        const response = await ApiUjianKhusus.put(
            `cfit/v2/banksoal/pohon/${aspek_detail_id.value}/soal/${soal_id.value}`,
            inputForm
        );
