...
const getData = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/v2/banksoal/pohon/${aspek_detail_id.value}/soal`);
...
const doDeleteData = async (id, index) => {
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            const response = await ApiUjianKhusus.delete(`cfit/v2/banksoal/pohon/${aspek_detail_id.value}/soal/${id}`);
            if (response.status) {
