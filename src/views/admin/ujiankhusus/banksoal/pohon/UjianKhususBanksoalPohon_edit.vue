...
const aspek_detail_id = ref(route.params.pohon_id);
const dataForm = ref({
    nama: "",
    desc: " ",
    urutan: 1,
...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/v2/banksoal/pohon/${aspek_detail_id.value}`);
...
const onSubmit = async (values) => {
    ...
    try {
        const response = await ApiUjianKhusus.put(`cfit/v2/banksoal/pohon/${aspek_detail_id.value}`, dataForm.value);
        // console.log(response);
        Toast.success("Info", "Data berhasil ditambahkan!");
        router.push({ name: "admin-ujiankhusus-banksoal-pohon" });
        return true;
    } catch (error) {
        console.error(error);
    }
