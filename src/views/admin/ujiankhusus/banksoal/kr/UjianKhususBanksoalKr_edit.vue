...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`ujiankhusus/kr/banksoal/index/${kr_id.value}`);
        ...
        dataForm.value.random_pilihanjawaban = response.data.random_pilihanjawaban == true ? true : false;
...
const onSubmit = async (values) => {
    ...
    values.status = dataForm.value.status
        ? true
        :false;
    values.random_pilihanjawaban = dataForm.value.random_pilihanjawaban
        ? true
        : false;
    ...
    try {
        const response = await ApiUjianKhusus.put(`ujiankhusus/kr/banksoal/index/${kr_id.value}`, dataFormSend);
        // console.log(response);
        Toast.success("Info", "Data berhasil ditambahkan!");
        router.push({ name: "admin-ujiankhusus-banksoal-kr" });
        return true;
    } catch (error) {
        console.error(error);
    }
                        ...
                        <div class="max-w-xs py-2">
                            <div class="form-control">
                                <label class="label cursor-pointer">
                                    ...
                                    <input type="checkbox" class="toggle" v-model="dataForm.random_pilihanjawaban" name="random_pilihanjawaban" />
                                </label>
                            </div>
                        </div>
                    </div>
