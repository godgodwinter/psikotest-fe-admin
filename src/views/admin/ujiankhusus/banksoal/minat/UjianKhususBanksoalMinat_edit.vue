...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`ujiankhusus/minatbakat/banksoal/index/${banksoal_minatbakat_id.value}`);
        ...
        dataForm.value.random_kelompok = response.data.random_kelompok == true ? true : false;
        dataForm.value.random_profesi = response.data.random_profesi == true ? true : false;
...
const onSubmit = async (values) => {
    ...
    values.status = dataForm.value.status
        ? true
        : false;
    values.random_profesi = dataForm.value.random_profesi
        ? true
        : false;
    values.random_kelompok = dataForm.value.random_kelompok
        ? true
        : false;
    ...
    let dataFormSend = {
        ...
        random_kelompok: values.random_kelompok,
        random_profesi: values.random_profesi,
    };
    try {
        const response = await ApiUjianKhusus.put(`ujiankhusus/minatbakat/banksoal/index/${banksoal_minatbakat_id.value}`, dataFormSend);
        // console.log(response);
        Toast.success("Info", "Data berhasil ditambahkan!");
        router.push({ name: "admin-ujiankhusus-banksoal-minat" });
        return true;
    } catch (error) {
        console.error(error);
    }
                ...
                <div class="space-y-4">
                        ...
                        <div class="max-w-xs py-2">
                            <div class="form-control">
                                <label class="label cursor-pointer">
                                    <span class="label-text">Status {{ dataForm.status }}</span>
                    ...
                    <div class="flex flex-col">
                        <div class="max-w-xs py-2">
                            <div class="form-control">
                                <label class="label cursor-pointer">
                                    <span class="label-text">Random Kelompok</span>
                                    <input type="checkbox" class="toggle" v-model="dataForm.random_kelompok" name="random_kelompok" />
                                </label>
                            </div>
                        </div>
                    </div>
                    <div class="flex flex-col">
                        <div class="max-w-xs py-2">
                            <div class="form-control">
                                <label class="label cursor-pointer">
                                    <span class="label-text">Random Profesi</span>
                                    <input type="checkbox" class="toggle" v-model="dataForm.random_profesi"
                                        name="random_profesi" />
                                </label>
                            </div>
                        </div>
