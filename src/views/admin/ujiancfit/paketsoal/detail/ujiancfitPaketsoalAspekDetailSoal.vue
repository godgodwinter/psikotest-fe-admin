...
const doDeleteData = async (id, index) => {
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            // console.log(id);
            const response = await ApiUjianKhusus.delete(`cfit/paketsoal/${paketsoal_id.value}/umum/${aspek_detail_id.value}/soal/banksoal/${banksoal_aspek_detail_id.value}/delete/${id}`);
            // if (response.status) {
