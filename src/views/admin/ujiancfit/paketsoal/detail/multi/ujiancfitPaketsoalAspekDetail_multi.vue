...
const getData = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/paketsoal/${paketsoal_id.value}/multi`);
        console.log('====================================');
...
const doDeleteData = async (id, index) => {
    ...
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            const response = await ApiUjianKhusus.delete(`cfit/paketsoal/${paketsoal_id.value}/multi/${id}`);
...
<template>
    <div>
        <article class="prose lg:prose-sm">
            <h1>MULTI - ASPEK DETAIL </h1>
            <h5>UJIAN CFIT</h5>
        </article>
        <div>
            <RouterLink :to="{ name: 'admin-ujiancfit-paketsoal', params: { paketsoal_id } }">
                <button class="btn btn-info btn-sm">Kembali ke PAKETSOAL CFIT</button>
            </RouterLink>
        </div>
        <span v-if="isLoading">
            <LoadingNavbar />
            ...
            <div class="card bg-base-100 border border-1 border-slate-500 p-4 max-w-xl" v-if="formEdit">
                ...
                <div>
                    <div class="form-control w-full max-w-md">
                        ...
                        <input type="text" placeholder="Mapel" class="input input-bordered w-full max-w-md"
                            v-model="dataForm.mapel" />
                        <label class="label">
                        ...
                        <div>
                            <div class=" flex flex-col">
                                <label class="label">
                                    <span class="label-text">Range Tanggal Generate Paket ke siswa yang akan ikut
                                        terupdate : </span>
                                </label>
                            ...
                            <vue-good-table :line-numbers="true" :columns="columns" :rows="data" :search-options="{
                                ...
                                <template #table-actions>
                                    <div class="space-x-1 space-y-1 gap-1">
                                        <router-link :to="{
                                            name: 'admin-ujiancfit-paketsoal-aspek_detail-multi-tambah', params: { paketsoal_id },
                                        }">
                                ...
                                <template #table-row="props">
                                    <span v-if="props.column.field == 'actions'">
                                        <div class="text-sm font-medium text-center flex justify-center space-x-1">
                                            ...
                                            <RouterLink
                                                :to="{ name: 'admin-ujiancfit-paketsoal-aspek_detail-multi-soal', params: { paketsoal_id, aspek_detail_id: props.row.khusus_banksoal_aspek_detail_id, banksoal_aspek_detail_id: props.row.khusus_banksoal_aspek_detail_id } }"
                                            ...
                                            <RouterLink
                                                :to="{ name: 'admin-ujiancfit-paketsoal-aspek_detail-multi-edit', params: { paketsoal_id, aspek_detail_id: props.row.khusus_banksoal_aspek_detail_id } }"
                                                v-if="props.row.khusus_banksoal_aspek_detail_id">
                                            ...
                                            <button class="btn btn-sm btn-danger"
                                                @click="doDeleteData(props.row.khusus_banksoal_aspek_detail_id, props.index)">
                                                <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5"
                                                    viewBox="0 0 20 20" fill="currentColor">
                                                    <path fill-rule="evenodd"
