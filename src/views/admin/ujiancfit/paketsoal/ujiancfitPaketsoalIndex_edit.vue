...
const getDataDetail = async () => {
    try {
        const response = await ApiUjianKhusus.get(`cfit/paketsoal/${paketsoal_id.value}`);
...
const onSubmit = async (values) => {
    ...
    try {
        const response = await ApiUjianKhusus.put(`CFIT/paketsoal/${paketsoal_id.value}`, dataFormSend);
        // console.log(response);
        Toast.success("Info", "Data berhasil diupdate!");
        router.push({ name: "admin-ujiancfit-paketsoal" });
        return true;
    } catch (error) {
        console.error(error);
    }
