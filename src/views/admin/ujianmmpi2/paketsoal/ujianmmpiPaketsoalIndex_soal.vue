...
const getData = async () => {
    try {
        const response = await ApiUjianKhusus.get(`mmpi2/paketsoal/${paketsoal_id.value}/get_soal`);
...
const doDeleteData = async (id, index) => {
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            const response = await ApiUjianKhusus.delete(`mmpi2/paketsoal/${paketsoal_id.value}/remove_soal/${id}`);
            if (response.status) {
...
const doDeleteDataAll = async (id, index) => {
    if (confirm("Apakah anda yakin menghapus data ini?")) {
        try {
            const response = await ApiUjianKhusus.delete(`mmpi2/paketsoal/${paketsoal_id.value}/add_soal/clear`);
            if (response.status) {
                Toast.warning("Berhasil", response.message);
                // Toast.success("Info", "Data berhasil dihapus!");
            } else {
