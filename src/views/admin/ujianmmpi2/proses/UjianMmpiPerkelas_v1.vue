...
const getData = async () => {
    try {
        console.log('====================================');
        console.log('#getData MMPI');
        console.log('====================================');
        isLoading.value = true;
        const response = await ApiUjianKhusus.get(`/mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`);
...
const doPilihPaket = () => {
    ...
    localStorage.setItem("ujianmmpi_paketsoal_aktif", JSON.stringify(tempDataSave))
    // console.log('====================================');
...
const doGenerateSiswa = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini (v3)?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`, dataFormSend);
...
const doDeleteProsesSiswa = async (id, proses_id) => {
    if (confirm("Apakah anda yakin menghapus data ujian ini?")) {
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.delete(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}`);
...
const doGenerateSiswaPerkelas = async (id, index) => {
    if (confirm("Apakah anda yakin generate data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`, dataFormSend);
            Toast.babeng("Berhasil", 'Data berhasil digenerate!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doDeleteProsesSiswaPerkelas = async (id) => {
    if (confirm("Apakah anda yakin menghapus data ujian ini?")) {
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.delete(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}`);
            Toast.babeng("Berhasil", 'Data berhasil di hapus!');
            getData();
        } catch (error) {
            isLoading.value = false;
...
const doCachingRedisPerSiswa = async (id, index) => {
    if (confirm("Apakah anda yakin caching data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/siswa/${id}/caching`);
...
const doCachingRedisPerKelas = async () => {
    if (confirm("Apakah anda yakin caching data ini?")) {
        ...
        try {
            isLoading.value = true;
            const response = await ApiUjianKhusus.post(`mmpi/proses/sekolah/${sekolah_id.value}/kelas/${getSekolahAktif.value.kelas_id}/caching`);
...
const dataTemp = JSON.parse(localStorage.getItem("ujianmmpi_paketsoal_aktif"));
    ...
    <span v-else>
        <div>
            <article class="prose lg:prose-sm">
                <h1>UJIAN MMPI KELAS {{ kelas_nama }} </h1>
                <h5>UJIAN MMPI - V1</h5>
            </article>
        ...
        : " - " }}
                                            ...
                                            <!-- {{ props.row.mmpi?.tgl_batas_terakhir
                                                ? moment(props.row.mmpi?.tgl_batas_terakhir).format("DD MMMM YYYY
                                                                                        HH: mm: ss"):" - " }} -->
                                        </span>
